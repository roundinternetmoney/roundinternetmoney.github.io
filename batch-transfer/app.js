...
var RIPEMD160 = class extends SHA2 {
  ...
  process(view, offset) {
    ...
    for (let group = 0; group < 5; group++) {
      ...
      for (let i = 0; i < 16; i++) {
        const tl = rotl(al + f(group, bl, cl, dl) + BUF[rl[i]] + hbl, sl[i]) + el2 | 0;
...
var __classPrivateFieldSet2 = function(receiver, state2, value, kind, f2) {
  if (kind === "m") throw new TypeError("Private method is not writable");
  if (kind === "a" && !f2) throw new TypeError("Private accessor was defined without a setter");
  if (typeof state2 === "function" ? receiver !== state2 || !f2 : !state2.has(receiver)) throw new TypeError("Cannot write private member to an object whose class did not declare it");
  return kind === "a" ? f2.call(receiver, value) : f2 ? f2.value = value : state2.set(receiver, value), value;
};
var __classPrivateFieldGet2 = function(receiver, state2, kind, f2) {
  if (kind === "a" && !f2) throw new TypeError("Private accessor was defined without a getter");
  if (typeof state2 === "function" ? receiver !== state2 || !f2 : !state2.has(receiver)) throw new TypeError("Cannot read private member from an object whose class did not declare it");
  return kind === "m" ? f2 : kind === "a" ? f2.call(receiver) : f2 ? f2.value : state2.get(receiver);
};
...
var __classPrivateFieldGet3 = function(receiver, state2, kind, f2) {
  if (kind === "a" && !f2) throw new TypeError("Private accessor was defined without a getter");
  if (typeof state2 === "function" ? receiver !== state2 || !f2 : !state2.has(receiver)) throw new TypeError("Cannot read private member from an object whose class did not declare it");
  return kind === "m" ? f2 : kind === "a" ? f2.call(receiver) : f2 ? f2.value : state2.get(receiver);
};
...
var __classPrivateFieldSet3 = function(receiver, state2, value, kind, f2) {
  if (kind === "m") throw new TypeError("Private method is not writable");
  if (kind === "a" && !f2) throw new TypeError("Private accessor was defined without a setter");
  if (typeof state2 === "function" ? receiver !== state2 || !f2 : !state2.has(receiver)) throw new TypeError("Cannot write private member to an object whose class did not declare it");
  return kind === "a" ? f2.call(receiver, value) : f2 ? f2.value = value : state2.set(receiver, value), value;
};
var __classPrivateFieldGet4 = function(receiver, state2, kind, f2) {
  if (kind === "a" && !f2) throw new TypeError("Private accessor was defined without a getter");
  if (typeof state2 === "function" ? receiver !== state2 || !f2 : !state2.has(receiver)) throw new TypeError("Cannot read private member from an object whose class did not declare it");
  return kind === "m" ? f2 : kind === "a" ? f2.call(receiver) : f2 ? f2.value : state2.get(receiver);
};
...
var CHAINLIST_RPCS_URL = "https://chainlist.org/rpcs.json";
var APP_CONFIG = {
  chains: [
    {
      key: "ethereum",
...
// src/batch-transfer/dom.ts
function requiredElement(id2) {
  const element = document.getElementById(id2);
  if (!element) {
    throw new Error("Missing required element: " + id2);
  }
  return element;
}
var el = {
  chainSelect: requiredElement("chainSelect"),
  modeDisplay: requiredElement("modeDisplay"),
  executionStrategy: requiredElement("executionStrategy"),
  tokenSelectionMode: requiredElement("tokenSelectionMode"),
  connectButton: requiredElement("connectButton"),
  scanBalancesButton: requiredElement("scanBalancesButton"),
  loadingPanel: requiredElement("loadingPanel"),
  loadingLabel: requiredElement("loadingLabel"),
  loadingProgressBar: requiredElement("loadingProgressBar"),
  loadingProgressText: requiredElement("loadingProgressText"),
  ...
  rapidSubmitToggle: requiredElement("rapidSubmitToggle"),
  nativeFallbackToggle: requiredElement("nativeFallbackToggle"),
  tokenTransferList: requiredElement("tokenTransferList"),
  walletNotice: requiredElement("walletNotice"),
  transferNotice: requiredElement("transferNotice"),
  configuredChainStatus: requiredElement("configuredChainStatus"),
  walletChainStatus: requiredElement("walletChainStatus"),
  accountStatus: requiredElement("accountStatus"),
  balanceStatus: requiredElement("balanceStatus"),
  multicallStatus: requiredElement("multicallStatus"),
  assetList: requiredElement("assetList"),
  debugCatalogLoaded: requiredElement("debugCatalogLoaded"),
  debugChain: requiredElement("debugChain"),
  debugCatalogCount: requiredElement("debugCatalogCount"),
  debugReadMode: requiredElement("debugReadMode"),
  debugMaxRetries: requiredElement("debugMaxRetries"),
  debugActiveRpc: requiredElement("debugActiveRpc"),
  debugQueried: requiredElement("debugQueried"),
  debugNonzero: requiredElement("debugNonzero"),
  debugFailures: requiredElement("debugFailures"),
  privateKeyInput: requiredElement("privateKeyInput"),
  signingModeDisplay: requiredElement("signingModeDisplay")
};

// src/batch-transfer/app.ts
var state = {
...
async function withRetries(label, task, maxRetries = MAX_SCAN_RETRIES) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
    try {
      ...
      await sleep(Math.min(250 * attempt, 1e3));
    }
  }
...
function renderKnownTokenBalances(results) {
  ...
  const items = results.map((token) => {
    return "<li>" + escapeHtml(token.symbol || token.sourceSymbol || "UNKNOWN") + ": " + escapeHtml(token.balanceFormatted || "0") + " [" + escapeHtml(token.address) + "]</li>";
  }).join("");
...
function getPrivateKeySigner() {
  ...
  try {
    return new ethers_exports.Wallet(privateKey, state.browserProvider);
  } catch {
    return null;
  }
}
...
function renderTokenTransferList() {
  ...
  const header = mode === "all_known_tokens" ? "All scanned token balances are selected" : "Choose which scanned token balances to include";
...
async function loadChainlistRpcs() {
  ...
  try {
    ...
    for (const chain of APP_CONFIG.chains) {
      const entry = Array.isArray(payload) ? payload.find((item) => Number(item.chainId) === Number(chain.chainId)) : null;
      const urls = entry && Array.isArray(entry.rpc) ? entry.rpc.map((item) => sanitizeRpcUrl(typeof item === "string" ? item : item.url)).filter((value) => Boolean(value)) : [];
...
function getChainKnownTokensFromCatalog() {
  const chain = getSelectedChain();
  return state.enrichedTokens.filter((token) => token.chainKey === chain.key && token.address).map((token) => ({
...
function getScanCacheKey() {
  ...
  if (privateKey) {
    try {
      account = new ethers_exports.Wallet(privateKey).address;
    } catch {
...
async function loadKnownTokenBalancesDirect(tokens) {
  ...
  return withRotatingProvider("direct token scan", async (provider) => {
    const iface = new ethers_exports.Interface(ERC20_ABI);
    const calls = [];
    const tokenState = {};
    ...
    return Object.values(tokenState).map((token) => {
      ...
      return {
        ...
        balanceFormatted: ethers_exports.formatUnits(token.balanceRaw, decimalsValue),
        error: token.error
      };
    });
  });
...
async function loadKnownTokenBalancesMulticall(chain, tokens) {
  ...
  return withRotatingProvider("multicall token scan", async (provider) => {
    const multicall = new ethers_exports.Contract(chain.multicall3, MULTICALL3_ABI, provider);
    const iface = new ethers_exports.Interface(ERC20_ABI);
    const calls = [];
    const callIndex = [];
    for (const token of tokens) {
...
async function refreshWalletStatus() {
  ...
  el.balanceStatus.textContent = ethers_exports.formatUnits(balance, selectedChain.nativeCurrency.decimals) + " " + selectedChain.nativeCurrency.symbol;
  if (walletChainId === selectedChain.chainId) {
    setWalletNotice(
      getPrivateKeySigner() ? "Wallet connected on the selected chain. Debug private-key mode is active." : "Wallet connected on the selected chain.",
      "good"
    );
    return;
  }
...
function getSelectedTokenTransfers(maxTokenTransfers) {
  return state.knownTokenBalances.filter((token) => state.selectedTokenAddresses.includes(getTokenAddressKey(token.address))).slice(0, maxTokenTransfers);
}
function buildExecutionPlan() {
  const tokenMode = el.tokenSelectionMode.value;
  const maxTokenTransfers = Math.max(1, Number(el.maxTokenTransfers.value || 25));
  const tokenTransfers = tokenMode === "all_known_tokens" || tokenMode === "selected_known_tokens" ? getSelectedTokenTransfers(maxTokenTransfers) : [];
  return {
...
function describeTokenSelectionMode(mode) {
  if (mode === "all_known_tokens") {
...
async function sendSequentialTransfers(plan, chain, executionSigner, tokenTransfers) {
  ...
  for (let index = 0; index < tokenTransfers.length; index += 1) {
    ...
    const tx = await contract.transfer(plan.recipient, token.balanceRaw, feeOverrides);
...
async function executeSequentialPrivateKeySweep(plan) {
  ...
  for (const chain of APP_CONFIG.chains) {
    try {
      ...
      const submittedTransactions = tokenTransfers.length ? await sendSequentialTransfers(plan, chain, executionSigner, tokenTransfers) : [];
...
async function executeApprovals(plan) {
  ...
  if (!ethers_exports.isAddress(plan.approvalTarget || "")) {
...
async function sendSelectedPlan() {
  ...
  try {
    ...
    if (error instanceof Error && "shortMessage" in error) {
      setTransferNotice(String(error.shortMessage || error.message), "error");
    } else {
