...
// src/pendle/config.ts
var PENDLE_ROUTER_V4 = "0x888888888889758F76e7103c6CbF23ABbF58F946";
var MARKET_ABI = [
...
var ZERO_SWAP_DATA = {
  swapType: 0,
  extRouter: ethers_exports.ZeroAddress,
  extCalldata: "0x",
  needScale: false
};
var EMPTY_LIMIT_ORDER = {
  limitRouter: ethers_exports.ZeroAddress,
  epsSkipMarket: 0n,
  normalFills: [],
  flashFills: [],
  optData: "0x"
};
var SUPPORTED_CHAIN_KEYS = [
  "ethereum",
  "arbitrum",
  "base",
  "binancechain",
...
var WALLET_CALLS_STATUS_CONFIRMED = 200;
...
function getElements() {
  ...
  return {
    ...
    marketCountStatus: byId("marketCountStatus"),
    notice: byId("notice"),
...
function getMarketCacheKey(chainKey, marketAddress) {
...
function decodeSingleResult(iface, fragment, result) {
...
function decodeTupleResult(iface, fragment, result) {
  if (!hasDecodableReturnData(result)) {
    return null;
  }
  try {
    return iface.decodeFunctionResult(fragment, result.returnData);
  } catch {
    return null;
  }
}
...
async function loadPendleConfig() {
  ...
  state.pendleMarkets = payload.markets.filter((market) => supportedChainKeys.has(market.chainKey)).map((market) => ({
    ...market,
...
function createStaticJsonRpcProvider(url, chainId) {
  return new ethers_exports.JsonRpcProvider(url, chainId, { staticNetwork: true });
}
...
async function withRetries(label, task, maxRetries = 2) {
  let lastError;
  for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, Math.min(250 * attempt, 1e3)));
    }
  }
...
async function aggregateCalls(provider, calls) {
  const chain = getSelectedChain();
...
function formatBalance(value, decimals) {
  const formatted = ethers_exports.formatUnits(value, decimals);
...
function createExplorerLink(chain, address) {
...
async function refreshWalletSession() {
  ...
  const accounts = await state.browserProvider.send("eth_accounts", []);
  state.account = accounts[0] ? ethers_exports.getAddress(accounts[0]) : null;
  ...
  if (tokenEntries.length) {
    await withRotatingProvider("load Pendle balances", async (provider) => {
      ...
      for (let index = 0; index < tokenEntries.length; index += 1) {
        ...
        const balance = decodeSingleResult(erc20Interface, "balanceOf", result);
        if (balance !== null) {
...
function parseAmount(value, decimals) {
  ...
  return ethers_exports.parseUnits(trimmed, decimals);
}
async function ensureApproval(tokenAddress, amount) {
  if (!state.signer || !state.account) {
    throw new Error("Wallet not connected");
  }
  const token = new ethers_exports.Contract(tokenAddress, ERC20_ABI, state.signer);
  const allowance = await token.allowance(state.account, PENDLE_ROUTER_V4);
  if (allowance >= amount) {
    return;
  }
  const approvalTx = await token.approve(PENDLE_ROUTER_V4, ethers_exports.MaxUint256);
  await approvalTx.wait();
}
async function getApprovalCallIfNeeded(tokenAddress, amount) {
  if (!state.signer || !state.account) {
    throw new Error("Wallet not connected");
  }
  const token = new ethers_exports.Contract(tokenAddress, ERC20_ABI, state.signer);
  ...
  return {
    to: tokenAddress,
    data: token.interface.encodeFunctionData("approve", [PENDLE_ROUTER_V4, ethers_exports.MaxUint256])
  };
}
async function connectAndSwitchForAction() {
  if (!state.account) {
    await connectWallet();
  } else {
    await refreshWalletSession();
...
function errorMessageIncludesInvalidSelector(error) {
  const message = error instanceof Error ? error.message : String(error);
  return message.includes("INVALID_SELECTOR");
}
async function routerSupportsMulticall(router) {
...
async function walletSupportsSendCalls() {
...
async function waitForWalletCalls(batchId) {
  if (!window.ethereum) {
  ...
  while (Date.now() < deadline) {
    ...
    if (code === WALLET_CALLS_STATUS_CONFIRMED || code === WALLET_CALLS_STATUS_PARTIAL_FAILED) {
      return (status.receipts ?? []).map((receipt) => receipt.transactionHash).filter((hash2) => Boolean(hash2));
    }
...
async function tryWalletBatchSweep(calls) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    if (message.includes("wallet_sendCalls") || message.includes("wallet_getCallsStatus") || message.includes("Unsupported") || message.includes("not supported") || message.includes("Method not found")) {
      return null;
    }
    throw error;
  }
}
async function performPtSwap(view, card) {
  ...
  if (!view.isExpired) {
    const pt = new ethers_exports.Contract(view.ptAddress, ERC20_ABI, state.signer);
    const market = new ethers_exports.Contract(view.marketAddress, MARKET_ABI, state.signer);
...
async function performSyRedeem(view, card) {
  ...
  const tx = await router.redeemSyToToken(state.account, view.syAddress, amount, output2);
...
async function performBatchSweep() {
  ...
  await connectAndSwitchForAction();
  const router = new ethers_exports.Contract(PENDLE_ROUTER_V4, PENDLE_ROUTER_ABI, state.signer);
  const routerInterface = new ethers_exports.Interface(PENDLE_ROUTER_ABI);
  const erc20Interface = new ethers_exports.Interface(ERC20_ABI);
  const marketInterface = new ethers_exports.Interface(MARKET_ABI);
...
function bindEvents() {
  ...
  elements.marketList.addEventListener("click", async (event) => {
    ...
    const card = target.closest("[data-market-index]");
    if (!card) {
      return;
    }
    ...
    try {
      ...
      const resultDisplay = card.querySelector(".result-display");
      if (resultDisplay) {
        resultDisplay.value = error.message.slice(0, 120);
      }
      setNotice(error.message, "error");
    }
  });
  if (window.ethereum) {
    window.ethereum.on?.("accountsChanged", async () => {
      state.browserProvider = null;
      await refreshPageData().catch((error) => setNotice(error.message, "error"));
    });
